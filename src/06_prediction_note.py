...
print("=" * 60)
print("ÉTAPE 06 - PRÉDICTION DE LA NOTE")
print("=" * 60)
...
print(
    f"Avis disponibles après nettoyage : {len(df)}"
)
...
if len(df) > MAX_REVIEWS:
    ...
    print(
        f"\nÉchantillonnage de {MAX_REVIEWS} avis..."
    )
...
print(
    f"Nombre d'avis utilisés : {len(df)}"
)
...
print(
    f"Entraînement : {len(X_train)} avis"
)

print(
    f"Test : {len(X_test)} avis"
)
...
cm_df = pd.DataFrame(
    cm,
    index=[f"Réel {x}" for x in labels],
    columns=[f"Prédit {x}" for x in labels]
)
...
nouveaux_avis = [
    ...
    "Very bad product, terrible taste and poor quality.",
    "The product is okay, nothing special.",
    "I really like this product and I will buy it again.",
